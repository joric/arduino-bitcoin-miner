...
int serial_read(HANDLE hSerial, char * buf, int size);
int serial_write(HANDLE hSerial, char * buf, int size);

struct ArduinoSerial {
	HANDLE hSerial;
	void begin(int speed) { serial_open(&hSerial, speed); }
	int available() { return 1; }
	int readBytes(char * buf, int size) { return serial_read(hSerial, buf, size); }
	int write(char * buf, int size) { return serial_write(hSerial, buf, size); }
	void println() {};
	void print(char * s) {};
	void print(char c) { serial_write(hSerial, &c, 1); };
}; 
...
int serial_open(HANDLE * pSerial, int speed) {
	...
	if (GetCommState(hSerial, &dcbSerialParams) == 0) {
		print("Error getting device state\n");
		CloseHandle(hSerial);
		return -1;
	}
	...
	if(SetCommState(hSerial, &dcbSerialParams) == 0) {
		print("Error setting device parameters\n");
		CloseHandle(hSerial);
		return -1;
	}
